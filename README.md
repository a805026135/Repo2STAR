...
<!-- 截图占位：登录页面 -->
<!-- ![GitHub 登录](docs/screenshots/login.png) -->
...
<!-- 截图占位：导入仓库弹窗 -->
<!-- ![导入仓库](docs/screenshots/import-repos.png) -->
...
<!-- 截图占位：分析结果页面 -->
<!-- ![分析结果](docs/screenshots/analysis.png) -->
...
<!-- 截图占位：STAR 素材库 -->
<!-- ![STAR 素材库](docs/screenshots/star-points.png) -->

<!-- 截图占位：STAR 卡片编辑 -->
<!-- ![STAR 编辑](docs/screenshots/star-edit.png) -->
...
<!-- 截图占位：JD 匹配输入 -->
<!-- ![JD 输入](docs/screenshots/jd-input.png) -->

<!-- 截图占位：匹配结果 -->
<!-- ![匹配结果](docs/screenshots/match-result.png) -->
...
<!-- 截图占位：模板展示 -->
<!-- ![模板展示](docs/screenshots/templates.png) -->

<!-- 截图占位：AI 润色 -->
<!-- ![AI 润色](docs/screenshots/ai-polish.png) -->
...
<!-- 截图占位：面试准备 -->
<!-- ![面试准备](docs/screenshots/interview.png) -->
...
<!-- 截图占位：Agent 日志 -->
<!-- ![Agent 日志](docs/screenshots/logs.png) -->
...
│   │                                    #   fullPipeline(): 分析 → STAR → 变更检测
│   │                                    #   matchJob(): 项目筛选 → STAR 重写
│   │                                    #   synthesizeUserProfile(): 跨仓库技能合成
│   │                                    #   handleWebhookPush(): Webhook 防抖处理
│   │                                    #   recordFeedback(): 用户反馈驱动迭代
│   ├── db/
│   │   └── schema.js                    # SQLite 初始化 + 10 张表建表语句
│   │                                    #   users, repos, analyses, star_points,
│   │                                    #   job_matches, agent_logs, webhook_events,
│   │                                    #   skill_profiles, user_settings,
...
│       ├── analyzer.js                  # 仓库分析引擎（数据采集 → 智能采样 → AI 分析 → 变更检测）
...
│
├── src/                                 # 前端源码
│   ├── App.jsx                          # 根组件 + 页面路由（登录/控制台/编辑器）
│   ├── main.jsx                         # React 入口
│   ├── index.css                        # 全局样式 + Tailwind 自定义类
│   ├── components/
│   │   ├── AgentChat.jsx                # AI 对话助手（多阶段引导 + 文件上传 + 自由对话）
│   │   ├── AgentLogView.jsx             # Agent 操作日志查看
│   │   ├── ExportView.jsx               # 多格式导出面板（JSON/Markdown/PR/Notion/飞书）
│   │   ├── InterviewPrepView.jsx        # 面试准备（选仓库 → 生成题目 → 展开回答）
│   │   ├── JobMatcherView.jsx           # JD 匹配（输入 JD → 匹配结果 → 历史记录）
│   │   ├── Navbar.jsx                   # 顶部导航栏（用户信息 + 退出）
│   │   ├── RepoOverview.jsx             # 仓库总览（卡片列表 + 导入弹窗 + 分析/关注/移除）
│   │   ├── ResumeEditor.jsx             # 简历编辑器（7 个可折叠模块 + 证件照上传 + AI 润色）
│   │   ├── ResumePreview.jsx            # 简历实时预览（A4 尺寸 + 缩放控制）
│   │   ├── SettingsView.jsx             # 用户设置页
│   │   ├── Sidebar.jsx                  # 侧边栏导航（9 个功能入口 + 统计数据）
│   │   ├── SkillProfileView.jsx         # 技能画像（核心优势 + 技能详情 + 技术模式）
│   │   ├── StarPointsView.jsx           # STAR 素材库（双栏卡片 + 搜索过滤 + 反馈）
│   │   ├── TemplateSelector.jsx         # 模板选择器（6 套模板预览）
│   │   └── templates/                   # 6 套简历模板组件
│   │       ├── ClassicTemplate.jsx      #   优雅传统
│   │       ├── ExecutiveTemplate.jsx    #   卡片北欧风
│   │       ├── FreshTemplate.jsx        #   色彩创意
│   │       ├── MinimalistTemplate.jsx   #   极简一页
│   │       ├── ModernTemplate.jsx       #   现代极简
│   │       └── TechTemplate.jsx         #   侧边栏深色
│   ├── data/
│   │   ├── defaultResume.js             # 默认简历数据结构 + 各模块工厂函数
│   │   └── templates.js                 # 模板注册表（ID、名称、描述、预览色）
│   ├── pages/
│   │   ├── Dashboard.jsx                # 主控制台（侧边栏路由 + 数据预加载）
│   │   └── LoginPage.jsx                # 登录页（GitHub OAuth + 功能介绍卡片）
│   ├── services/
│   │   ├── api.js                       # 前端 API 封装（Token 管理 + 全部接口）
│   │   ├── exportDocx.js               # Word 导出逻辑（docx 库构建文档）
│   │   ├── exportPdf.js                # PDF 导出逻辑（html2canvas + jsPDF）
│   │   ├── fileParser.js               # 文件解析（PDF/Word/图片/文本分类处理）
│   │   └── mimoApi.js                  # MiMo API 适配 + AI 润色 + 数据提取
│   └── store/
│       ├── AppContext.jsx               # 全局应用状态（用户/仓库/STAR/匹配/日志/设置）
│       └── ResumeContext.jsx            # 简历编辑状态（数据/模板/章节管理）
│
├── data/                                # SQLite 数据库文件（.gitignore）
│   └── repo2star.db                     # 运行时自动生成
├── index.html                           # HTML 入口（Google Fonts 预加载）
├── package.json                         # 项目配置
├── vite.config.js                       # Vite 配置（API 代理 + MiMo 代理）
├── tailwind.config.js                   # Tailwind 主题（自定义颜色 + 字体）
